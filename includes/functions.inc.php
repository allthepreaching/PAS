...
function emptyInputUpload($name, $link, $code, $date)
{
    $result;
    if (empty($name) || empty($link) || empty($code) || empty($date)) {
        $result = true;
    } else {
        $result = false;
    }
...
function uploadVideo($conn, $name, $link, $code, $date)
{
    $sql = "INSERT INTO pasUpload (name, link, code, date) VALUES (?, ?, ?, ?);";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
    ...
    mysqli_stmt_bind_param($stmt, "ssss", $name, $link, $code, $date);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
